...
async def main():
    ...
    for _ in range(15):
        for i, tab in enumerate(driver.tabs):
            ...
            await tab.sleep(random.uniform(0.005, 0.03))
...
async def perf_watcher():
    ...
    while True:
        ...
        logger.debug(
            f"[ {time.perf_counter() - start_time} ]   coroutines: {len(all_tasks)} (of which: {len_done} done) / (threads: {threading.active_count()}"
        )
